...
void chancorrel(std::string const& filename="data_evb01_run10388_2_20240112T235253_tpcdecode.root", 
             size_t ievcount=0, 
             size_t tickmin=0,
             size_t tickmax=3415, 
...

{
  ...
  TCanvas *mycanvas = (TCanvas*) new TCanvas("correl","correl",12000,12000);
	...
	if (!rawdigits.empty())
	  {
	    ...
	    //slope->Draw("colz");
	    gPad->Print("correl2d.png");
	    TFile outfile("correl2d.root","RECREATE");
	    correl->Write();
	    outfile.Close();
	  }
      }
