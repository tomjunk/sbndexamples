...
void chanrangefftcnr_mean(std::string filename="tpcdecode_data_evb03_run11505_24_20240304T182922.root", int debuglevel=0)
{
  readchanmapfile();
  ...
  for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
		...
		for (const auto& pp : fp.second)  // loop over planes
		  {
		    ...
		    if (debuglevel>1 && ievent == 1)
		      {
			cout << "Mean on FEMBID: " << fp.first << " Plane: " << pp.first << " Tick: " << itick << " " << adcmed << endl; 
		      }
		  }
	      }
	  }
	...
	for (size_t ichan=0;ichan<nrawdigits; ++ichan) 
	  { 
		...
		adcsub.push_back(adcnonsub.back() - meanmap[fembident][iplane][itick]);
		if (debuglevel > 2)
		  {
		    if (ievent == 1 && ic == 1000)
		      {
			cout << "debug chan 1000: " << itick << " " << adcnonsub.back() << " " << adcsub.back() << " " << meanmap[fembident][iplane][itick] << endl;
		      }
		  }
