// compile with:  clang++ -std=c++17 -o checkmap_v22 checkmap_v22.C
...
int main(int argc, char **argv)
{
  ...
  std::cout << sizeof(c) << " " << 11276*sizeof(c) << std::endl;
  std::cout << sizeof(std::string) << std::endl;

  for (size_t i=0; i<11276; ++i)
    {
      auto ci = GetChanInfoFromOfflChan(i);
      if (!ci.valid)
	{
	  std::cout << "Channel not in map: " << i << std::endl;
	  continue;
	}
