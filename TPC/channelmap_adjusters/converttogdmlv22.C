// Update channel map from the GDML v2.0 (channel map v1) to
// GDML v2.2 (channel map v2).
...
int main(int argc, char **argv)
{
  ...
  while (std::getline(inFile,line)) {
    ...
    int nc = c.offlchan;          // new channel number
    if (nc > 4799) nc += 6;       // skip 6 missing wires on the East
    if (nc > 10437) nc += 6;      // and 6 missing wires on the West
    c.offlchan = nc;
